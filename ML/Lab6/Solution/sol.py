############################################################################################
# Copyright (C) 2024-2026 by Sandro Cumani                                                 #
#                                                                                          #
...
if __name__ == '__main__':
    ...
    for prior, Cfn, Cfp in [(0.5, 1, 1), (0.8, 1, 1), (0.5, 10, 1), (0.8, 1, 10)]:
        ...
        print('DCF (non-normalized): %.3f' % (compute_empirical_Bayes_risk_binary(
            evalset_predictions_binary, evalset_labels_binary, prior, Cfn, Cfp, normalize=False)))
        print('DCF (non-normalized, multiclass code): %.3f' % (compute_empirical_Bayes_risk(
            evalset_predictions_binary,
            evalset_labels_binary,
        ...
        print('DCF (normalized): %.3f' % (compute_empirical_Bayes_risk_binary(
            evalset_predictions_binary, evalset_labels_binary, prior, Cfn, Cfp)))
        print('DCF (normalized, multiclass code): %.3f' % (compute_empirical_Bayes_risk(
            evalset_predictions_binary,
            evalset_labels_binary,
            numpy.array([1-prior, prior]), # Class 1 is the second element for multiclass
            numpy.array([[0, Cfn], [Cfp, 0]]))))
        minDCF, minDCFThreshold = compute_minDCF_binary_slow(evalset_llr_binary, evalset_labels_binary, prior, Cfn, Cfp, returnThreshold=True)
        print('MinDCF (normalized, slow): %.3f (@ th = %e)' % (minDCF, minDCFThreshold))
        minDCF, minDCFThreshold = compute_minDCF_binary_fast(evalset_llr_binary, evalset_labels_binary, prior, Cfn, Cfp, returnThreshold=True)
        print('MinDCF (normalized, fast): %.3f (@ th = %e)' % (minDCF, minDCFThreshold))

    # ROC plot - uncomment the commented lines to see the plot
    Pfn, Pfp, _ = compute_Pfn_Pfp_allThresholds_fast(evalset_llr_binary, evalset_labels_binary)
    ...
    for effPrior in effPriors:
        # Alternatively, we can compute actDCF directly from compute_empirical_Bayes_risk_binary_llr_optimal_decisions(evalset_llr_binary, evalset_labels_binary, effPrior, 1.0, 1.0)
        evalset_predictions_binary = compute_optimal_Bayes_binary_llr(evalset_llr_binary, effPrior, 1.0, 1.0)
        actDCF.append(compute_empirical_Bayes_risk_binary(evalset_predictions_binary, evalset_labels_binary, effPrior, 1.0, 1.0))
        minDCF.append(compute_minDCF_binary_fast(evalset_llr_binary, evalset_labels_binary, effPrior, 1.0, 1.0))
    matplotlib.pyplot.figure(1)
    matplotlib.pyplot.plot(effPriorLogOdds, actDCF, label='actDCF Cls 1', color='r')
    matplotlib.pyplot.plot(effPriorLogOdds, minDCF, label='DCF Cls 1', color='b')
    matplotlib.pyplot.ylim([0, 1.1])
    #matplotlib.pyplot.show()
            
    evalset_llr_binary = numpy.load('../Data/evalset_llr_binary_2.npy')
    evalset_labels_binary = numpy.load('../Data/evalset_labels_binary.npy')
    ...
    matplotlib.pyplot.plot(effPriorLogOdds, actDCF, label='actDCF Cls 2', color='y')
    matplotlib.pyplot.plot(effPriorLogOdds, minDCF, label='DCF Cls 2', color='c')
    matplotlib.pyplot.ylim([0, 1.1])
    ...
    evalset_posteriors = compute_posteriors(evalset_ll, prior)
    evalset_predictions = compute_optimal_Bayes(evalset_posteriors, costMatrix)
    print(compute_confusion_matrix(evalset_predictions, evalset_labels))
    print('Emprical Bayes risk: %.3f' % compute_empirical_Bayes_risk(
        evalset_predictions, evalset_labels, prior, costMatrix, normalize=False))
    print('Normalized emprical Bayes risk: %.3f' % compute_empirical_Bayes_risk(
        evalset_predictions, evalset_labels, prior, costMatrix, normalize=True))

    print()
    print('Classifier 2')
    evalset_ll = numpy.load('../Data/evalset_ll_2.npy')
    evalset_labels = numpy.load('../Data/evalset_labels.npy')
    ...
    # Uniform costs and prior
    
    print()
    print("-"*40)
    print()
    print("Multiclass task - uniform costs and prior")

    prior = numpy.ones(3)/3.0
